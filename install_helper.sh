...
# Update backend/.env
if grep -q "SUPABASE_URL=<stratum>" backend/.env; then
...
if grep -q "SUPABASE_SERVICE_KEY=<tcp>" backend/.env; then
...
if grep -q "PG_DATABASE_URL=<sha256>" backend/.env; then
...
if grep -q "OPENAI_API_KEY=<sha256>" backend/.env; then
...
if grep -q "JWT_SECRET_KEY=<sha256>" backend/.env; then
...
if grep -q "NEXT_PUBLIC_SUPABASE_URL=<poolbinance.com>" frontend/.env; then
